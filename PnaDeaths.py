...
# Pivot the DataFrame to have months as rows, years as columns, and Pneumonia Deaths as values
pivot_df = df.pivot('Month', 'Year', 'Pneumonia Deaths')
...
cbar.set_label('Pneumonia Deaths', rotation=270, labelpad=20)

# Find the cell with the highest number of Pneumonia deaths
max_value = pivot_df.max().max()
max_indices = pivot_df.stack().idxmax()
max_month, max_year = max_indices[0], max_indices[1]

# Highlight the cell with the highest number of Pneumonia deaths
...
ax.set_title(f'Monthly Pneumonia Deaths as of {current_date.strftime("%m/%d/%Y")}', fontsize=16, pad=30)
ax.text(0.5, 1.02, f'Highest Monthly Pneumonia Deaths: {int(max_value)} in {max_month} {max_year}',
